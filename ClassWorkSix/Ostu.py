...
def error():
    print('Usage:')
    print('python .\Ostu.py image_path oper_type')
    exit(-1)
...
if __name__ == '__main__':
    ...
    ost_img = None
    if oper_type == 'Ostf':
        ost_img = ostf_seg(img)
    elif oper_type == 'Iter':
        ost_img = iter_seg(img)
    else:
        print(oper_type, 'is not supported  algorithm!')
        print('Ostf and Iter are supported.')
        exit(-1)
    base_name = os.path.basename(image_path)
    targetRoot='./target/'
    write_name = os.path.join(targetRoot, oper_type + '_' + base_name)
    cv2.imwrite(write_name, ost_img)